...
- Release workflow (`.github/workflows/release.yml`) bumps the version on the repository's default branch, pushes it, then recreates a `release` branch where the rewritten `sketch.yaml` files are committed so tags and ZIPs include the release-ready examples.
